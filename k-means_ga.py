...
def main():
    ...
    input_individu = input("Berapa jumlah individu ? ")
    input_populasi = input("Berapa jumlah populasi ? ")
    ...
    for i in range(len(datafile)):
        ...
        kmean.addToDatas(d)
        clust.addData(d)
        kmean.addToClusters(clust)

    kmean.countDistancePerData()
    kmean.start(int(input_cluster))

    # varianceAnalysis = VarianceAnalysis(kmean.clusters, kmean.datas)
    # varianceAnalysis.analyze()

    # kmean.showCentroidPoint()
...
def showDefaultGraph(object):
    ...
    for i in range(len(object.clusters)):
        ...
        for j in range(len(object.clusters[i].data)):
            ...
            plt.scatter(object.clusters[i].data[j].x, object.clusters[i].data[j].y, color=(0, g, 0, 1))
        plt.scatter(rand.randrange(int(object.clusters[i].centroid_point.x) - 20, int(object.clusters[i].centroid_point.x) + 20, 3), rand.randrange(int(object.clusters[i].centroid_point.y) - 20, int(object.clusters[i].centroid_point.y) + 20, 3), color=(0, 0, 0, 1))
        r = r + 0.2
        g = g - 0.2
        b = b + 0.2
